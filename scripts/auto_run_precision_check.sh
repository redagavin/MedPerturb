...
if [ "$TEST_MODE" = true ]; then
    ...
    while [ $ITERATION -le $MAX_ITERATIONS ]; do
        ...
        COMPLETE_COUNT=$(ls -1 checkpoints/precision_check/precision_check_eval_${MODEL_SHORT}_gpu*_of_${TOTAL_GPUS}_COMPLETE 2>/dev/null | wc -l) || true
...
result_files = glob.glob('results/precision_check_eval_${MODEL_SHORT}_gpu*_of_${TOTAL_GPUS}.json')
